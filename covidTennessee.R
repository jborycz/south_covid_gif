...
pacman::p_load(tidyverse,dplyr,plyr,scales,lubridate,readxl,gdata,gplots,
               GGally,ggpubr,geofacet,ggmap,viridis,RColorBrewer,patchwork,ggpmisc,usmap,tigris)
...
countyfips <- read.csv(system.file("extdata", "county_fips.csv",package = "usmap"))
statefips <- read.csv(system.file("extdata", "state_fips.csv",package = "usmap"))
...
usfips <- left_join(countyfips,statefips, by.x = "abbr")
...
############################################################
south_counties_covid <- left_join(south_counties, covidByCounty, by.x = "countyFIPS")
south_counties_death <- left_join(south_counties, deathsByCounty, by.x = "countyFIPS")
...
start <- match("3.3.20",names(south_counties_covid))
end <-length(south_counties_covid[1,])
